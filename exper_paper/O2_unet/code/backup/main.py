...
network_map={'resnet101':ResNet101 , 'resnet18':ResNet18}
...
# Đường dẫn file log (ví dụ: ../results/log_resnet18_cifar10.txt)
log_file_path = os.path.join(args.result_dir, f"log_{args.network}_{args.dataset}_{args.noise_rate}.txt")

def write_log(text):
    # print(text) # Vẫn in ra màn hình để theo dõi
    with open(log_file_path, "a") as f: # Mở file ở chế độ "append" (ghi thêm)
        f.write(text + "\n")

# Ghi dòng tiêu đề đầu tiên cho file log
write_log(f"--- Bat dau thi nghiem: {args.network} trên {args.dataset} với tỷ lệ nhiễu {args.noise_rate} ---")
...
def first_stage(network,test_loader,filter_mask=None):
	...
	for epoch in range(1, args.n_epoch):
		...
		for i, (images, labels, indexes) in enumerate(train_loader_init):
			...
			loss_1.backward()
			optimizer1.step()
		print("STAGE 1 - epoch:%d" % epoch, "lr:%f" % lr, "train_loss:", globals_loss /ndata, "test_accuarcy:%f" % accuracy)
		log_msg = "STAGE 1 - epoch:%d  lr:%f  train_loss:%f  test_accuracy:%f" % (epoch, lr, globals_loss / ndata, accuracy)
		write_log(log_msg)
...
def second_stage(network,test_loader,max_epoch=250):
	...
	for epoch in range(1, max_epoch):
		...
		log_msg = "STAGE 2 - epoch:%d  lr:%f  train_loss:%f  test_acc:%f  noise_acc:%f" % (epoch, lr, globals_loss / ndata, accuracy, 1 - noise_accuracy)
		write_log(log_msg)
