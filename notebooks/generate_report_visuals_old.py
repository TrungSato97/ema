# c:\Users\truon\learning\ptit\research\trung\M_10_01_2025\code_v2\project\notebooks\generate_report_visuals.py
import os
import re
...
def create_main_table(df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    baseline_accs = df[df['iteration'] == 0].drop_duplicates(subset=['noise_ratio']).set_index('noise_ratio')['test_acc_orig'].to_dict()
...
def plot_class_balance(df: pd.DataFrame, report_dir: Path):
    ...
    for _, row in best_runs.iterrows():
        ...
        original_train_csv = Path(str(exp_path).split(f'noise_{noise_ratio}')[0]) / f"csvs/noise_{noise_ratio}/train.csv"
        
        kept_csv = exp_path / f"iteration_{best_iter}" / f"train_kept_{best_iter}.csv"

        if not original_train_csv.exists():
            logging.warning(f"Original train CSV not found, skipping class balance for noise {noise_ratio}: {original_train_csv}")
            continue
        if not kept_csv.exists():
            logging.warning(f"Kept CSV not found, skipping class balance for noise {noise_ratio}: {kept_csv}")
            continue
